...
export class Tokenizer {
    ...
    private prev_token_line: number = -1;

    constructor(
        grammar: TokenizerGrammar,
        encode: BufferEncoding | null = null,
    ) {
        ...
        this.FORMAT_STRINGS = {
            ...['n', 'N'].reduce((acc, p) => {
                let newEntries = Object.entries(this.QUOTES).reduce((acc2, [s, e]) => {
        ...
        const keywordTrieKeywords = [
            ...
            .filter(key => key.includes(" ") || Object.keys(grammar.SINGLE_TOKENS).some(single => key.includes(single)));
    ...
    convertTuplesToObject(tuples: Array<string | Array<string>>, defaultValue?: string) {
        let convertedObject: Record<string, string> = {};
        tuples.forEach(tuple => {
            if (typeof tuple === 'string') {
                convertedObject[tuple] = (defaultValue === undefined) ? tuple : defaultValue;
            } else {
    ...
    quotesToFormat(token_type: TokenType, arr: Array<string>) {
        ...
        let result: Record<string, [string, TokenType]> = {};
    ...
    tokenize(sql: string): Array<Token> {
        ...
        try {
            ...
            throw new Error(`Error tokenizing '${context}'\n\n Original error: ${(e as Error).message}`);
        }
    ...
    add(tokenType: TokenType, text: string | null = null) {
        ...
        if (
            ...
            && this.peeked !== ";"
            && (this.tokens.length === 1 || this.grammar.COMMAND_PREFIX_TOKENS.includes(this.tokens[this.tokens.length - 2].token_type))
        ) {
            let start = this.current;
            let tokens = this.tokens.length;
            this.scan(() => this.peeked === ";");
            this.tokens = this.tokens.slice(0, tokens);
            let text = this.sql.substring(start, this.current).trim();
            if (text) {
    ...
    advance(i = 1, alphaNum = false) {
        if (this.grammar.WHITE_SPACE[this.char] === TokenType.BREAK) { // check if the current char is a line break to increment the line number
            this.col = 1;
            this.line += 1;
        } else {
            this.col += i;
    ...
    scanKeywords() {
        ...
        while (chars) {
            ...
            size += 1;
            let end = this.current - 1 + size;

            if (end < this.size) {
                char = this.sql[end];
                single_token = single_token || this.grammar.SINGLE_TOKENS[char];
                let is_space = this.grammar.WHITE_SPACE[char];
        ...
        word = (!single_token && !this.grammar.WHITE_SPACE[chars[chars.length - 1]]) ? null : word;
    ...
    scanComment(comment_start: string): boolean {
        ...
        let comment_start_size = comment_start.length;
        let comment_end = this.COMMENTS[comment_start];

        if (comment_end) {
            // Skip the comment's start delimiter
            this.advance(comment_start_size);

            let comment_end_size = comment_end.length;
            while (!this.end && this.chars(comment_end_size) !== comment_end) {
                this.advance(1, true);
            }

            this.comments.push(this._text.substring(comment_start_size, this._text.length - comment_end_size + 1));
            this.advance(comment_end_size - 1);
        } else {
            while (!this.end && !(this.grammar.WHITE_SPACE[this.peeked]) && this.grammar.WHITE_SPACE[this.peeked] !== TokenType.BREAK) {
                this.advance(1, true);
            }
    ...
    scanNumber() {
        if (this.char === "0") {
            let peek = this.peek().toUpperCase();
            if (peek === "B") {
                return this.grammar.BIT_STRINGS ? this.scanBits() : this.add(TokenType.NUMBER);
            } else if (peek === "X") {
                return this.grammar.HEX_STRINGS ? this.scanHex() : this.add(TokenType.NUMBER);
            }
        }
        ...
        while (true) {
            if (!isNaN(Number(this.peek()))) {
                ...
                while (this.peek().trim() && !(this.peek() in this.grammar.SINGLE_TOKENS)) {
                    literal += this.peek().toUpperCase();
                    this.advance();
                }

                const token_type = this.grammar.KEYWORDS[this.grammar.NUMERIC_LITERALS[literal] || ""];
    ...
    scanBits() {
        ...
        if (isNaN(parsedValue) || parsedValue.toString(2) !== value.substring(2)) { //  If `value` can't be converted to a binary, fallback to tokenizing it as an identifier
    ...
    scanHex() {
        ...
        if (isNaN(parsedValue) || parsedValue.toString(16) !== value.substring(16)) { //  If `value` can't be converted to a hex, fallback to tokenizing it as an identifier
            this.add(TokenType.IDENTIFIER);
        } else {
    ...
    scanString(start: string): boolean {
        ...
        if (base) {
            if (isNaN(parseInt(text, base))) { //check, possible bug
                throw new Error(`Numeric string contains invalid characters from line ${this.line}:${this.start}`);
            }
        } else {
    ...
    scanVar() {
        while (true) {
            let char = this.peeked.trim();
            if (char && (this.grammar.VAR_SINGLE_TOKENS[char] || !this.grammar.SINGLE_TOKENS[char])) {
                this.advance(1, true);
            } else {
                break;
            }
        }
        let tokenType = this.tokens?.[this.tokens.length - 1]?.token_type === TokenType.PARAMETER ?
            TokenType.VAR :
    ...
    extractString(delimiter: string, escapes: Set<string> | null = null): string {
        let text = "";
        let delimSize = delimiter.length;
        escapes = escapes === null ? this.STRING_ESCAPES : escapes;

        while (true) {
            if (escapes.has(this.char) && (this.peeked === delimiter || escapes.has(this.peeked))) {
                ...
                if (this.current + 1 < this.size) {
                    ...
                    throw new Error(`Missing ${delimiter} from line ${this.line}:${this.current}`);
                }
            } else {
                ...
                if (this.end) {
                    throw new Error(`Missing ${delimiter} from line ${this.line}:${this.start}`);
                }
